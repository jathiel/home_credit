...
#############################################################
path_train = 'data/csv_files/train/'
...
def remove_strings(files):
    ...
    for i, file in enumerate(files):  # reading every file
        df = pd.read_csv(path_train + file)  # importing data into a dataframe
        ...
        df.to_csv(path_train + file, index=False)  # write new dataframe to file
